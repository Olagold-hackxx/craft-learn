...
export default function Header({ ...props }: Props) {
  ...
  return (
    <header {...props} className={`${props.className} flex w-screen flex-col  p-8 sm:py-4 z-50 border-b-[0.5px] border-[#EAEAE8A8] bg-primary shadow-md `}>
      <div className="container-xs mt-4 flex lg:px-5 md:px-5">
        <div className="flex w-full items-center justify-between gap-5 ">
          <div className="flex gap-10 md:w-full max-sm:flex-col">
            <Img src="logo.png" alt="Headerlogo" className="h-[52px] w-[170px] object-contain" />
            <Input
              shape="round"
              name="search"
              ...
              prefix={<Img src="icons/search-normal.png" alt="Contrast" className="h-[20px] w-[20px] object-contain" />}
              suffix={
                searchBarValue?.length > 0 ? (
                  <CloseSVG onClick={() => setSearchBarValue("")} fillColor="#6c6c6cff" />
                ) : null
              }
          ...
          <div className="flex  items-center justify-end gap-5 md:w-full">
            ...
            <div className="mr-4 flex items-center gap-2 self-end">
                <Img src="icons/teacher.png" alt="Thumbsup" className="h-[20px]" />
             <ConnectWallet />
  </div>
          </div>
        </div>
